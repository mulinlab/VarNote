...
//		this.outFormat = outFormat;
	...
	public void parserFormat(String format, boolean isOut) {
		...
		format = format.trim();
		if(format.indexOf(" ") != -1) throw new IllegalArgumentException("Format has space, please check " + format);
		
		String variable, sep;
		int index = 0;
		while((end = format.indexOf(VARIABLE_BEGIN)) != -1) {
			vnum = getVariableName(format, end + 1);
			if(!isOut && vnum > 0 && preNum > 0 && (preNum + 1) != vnum) throw new IllegalArgumentException("Please set format variable in order like $1,$2,$3...");
			if(vnum == ALLELE_INDENTIFIER) {
				...
				if(index < 1)  throw new IllegalArgumentException("Invalid format " + format + ", Please set format like $1|$2..$n"); 
	...
	public ValWithAllele getFieldValWithFormat(String val) {
		...
		for (int i = 0; i < size; i++) {
			...
			if(fv.seperator.equals("$begin")) {
				...
				if(i == 0) {
					if(val.indexOf(fv.seperator) == -1) throw new IllegalArgumentException(val + " don't match format for field " + fieldName);
			...
			if(fv.variable.equals(VARIABLE_BEGIN + ALLELE)) {
				...
				throw new IllegalArgumentException(val + " don't match format for field " + fieldName);
			} else {
...
//				sb.append(allele);
				...
				if(index == -1) {
					throw new IllegalArgumentException("Invalid format for field " + fieldName + ", Please use format like $2..$n or $3..$n and so on.");
				}
...
//					if(f.substring(index-3, index+1).equals(LOOP)) throw new IllegalArgumentException("Wrong format: " + f);
			...
			case '.':
				if(index + 4 > f.length()) throw new IllegalArgumentException("wrong format ofr field " + fieldName);
