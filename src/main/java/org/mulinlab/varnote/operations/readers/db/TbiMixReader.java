...
public final class TbiMixReader extends TbiReader{
	...
	public TbiMixReader(final String db, final boolean isCount) throws IOException {
		super(db, isCount);
	}
	...
	public TbiMixReader(final Database db, final boolean isCount) throws IOException {
		super(db, isCount);
	}
	...
	public boolean query(LocFeature query) throws IOException {
		if(!super.query(query)) return false;
		if(tindex == null) return false;
		
		 final int b_num = GlobalParameter.regionToBin4(query.beg, query.end);
		 if(b_num < 585) {
	...
	public TPair64[] getBins(final int min, final int max) {
        ...
        int i, l, n_off, n_bins = GlobalParameter.reg2bins(min, max, bins);
