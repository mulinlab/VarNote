 ...
 */
public final class MyBlockCompressedInputStream extends InputStream implements LocationAware {
     ...
     */
    public MyBlockCompressedInputStream(final SeekableStream strm, final boolean useJDK) {
        mFile = strm;
        mStream = null;
        if(!useJDK) {
			 blockGunzipper = new BlockGunzipper(new IntelInflaterFactory());
		} else {
			 blockGunzipper = new BlockGunzipper(new InflaterFactory());
		}
    }
    ...
    public void seek(final long pos) throws IOException {
        ...
        seek(compressedOffset, uncompressedOffset);
    }
    
    public void seek(final long compressedOffset, final int uncompressedOffset) throws IOException {
    	 final int available;
         ...
         if (uncompressedOffset > available || (uncompressedOffset == available && !eof())) {
             throw new IOException(INVALID_FILE_PTR_MSG + compressedOffset + ", " + uncompressedOffset + " for " + mFile.getSource());
         }
    ...
    public static void assertNonDefectiveFile(final File file) throws IOException {
        if (checkTermination(file) == FileTermination.DEFECTIVE) {
            throw new SAMException(file.getPath() + " does not have a valid GZIP block at the end of the file.");
        }
    }
