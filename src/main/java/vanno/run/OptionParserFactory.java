...
//		
		...
		parser.accepts("unzip", "Output compression. By default, the program will compress the output file using block gzip. ")
	...
	public static OptionParser getParserForAnno() {
		...
		parser.accepts("anno-out", "Output path to result file.")
	...
	public static OptionParser getParserForIndex() {
		...
		parser.acceptsAll(Arrays.asList("o", "out"), "Output directory. By default the output files will be written into the same folder as the input file. ")
	...
	public static OptionParser getParserForDatabase(OptionParser parser) {
		parser.acceptsAll(Arrays.asList("d", "d-file"),  "Local path or http/ftp address of indexed database files with either VarNote index (" + IndexType.VANNO.getExt() + " and " + IndexType.VANNO.getExtIndex() + ") or Tabix index("+ IndexType.TBI.getExtIndex() + ") in the same location " + 
		...
		parser.accepts("d-index", "Type of index. By default, the program will use " + BasicUtils.PRO_NAME + " index system, valid values are "+ IndexType.TBI.getName() +" or " + 
				IndexType.VANNO.getName() + " (multiple index should be separated by comma with no space). ")
			.withOptionalArg().describedAs("10:[str-list (" + IndexType.TBI.getName() + "," + IndexType.VANNO.getName() + ")]").withValuesSeparatedBy(',').ofType(String.class);   //查询的数据库文件
	...
	public static OptionParser getParserForOutput(OptionParser parser) {
		parser.acceptsAll(Arrays.asList("o", "out-floder"), "Output directory path. By default the program will create a fold in /path/to/query/query_name_out. ")
			.withOptionalArg().describedAs("31:[str]").ofType(String.class);  //输出文件
	
		parser.accepts("out-mode", "Output recording mode (default 2). 0 for \"only output query records\"; 1 for \"only output matched database records\"; 2 for \"output both query records and matched database records\". ")
