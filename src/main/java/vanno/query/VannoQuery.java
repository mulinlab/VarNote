...
public final class VannoQuery extends AbstractQuery {
	...
	public VannoQuery(final List<Database> dbs, final boolean useJDK, final Log log) {
	...
	public void init() throws IOException {
		...
		for (Database dbf : dbs) {
			if(dbf.getConfig().getIndexType() == IndexType.TBI) {
				readers.add(new TbiMixReader(dbf, useJDK, log));
			} else {
				readers.add(new VannoMixReader(dbf, useJDK, log));
			}
		}
	}
}
