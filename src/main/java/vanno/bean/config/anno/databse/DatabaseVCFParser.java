...
import main.java.vanno.bean.config.anno.InfoField;
...
public class DatabaseVCFParser extends AbstractDatababseParser{
	...
	public DatabaseVCFParser(final Database db,  final DatabseAnnoConfig config, final boolean isForceOverlap, final AnnoOutFormat annoOutFormat, final Log log, final boolean useJDK) {
		super(db, config, isForceOverlap, annoOutFormat);
		
		vcfParser = new VCFParser(db.getDbPath(), log, useJDK, FileType.BGZ);
