...
public final class LineIteratorImpl extends AbstractIterator<String> implements LineIterator, Closeable {
    ...
    public LineIteratorImpl(final String path, final boolean useJDK, final FileType fileType) throws IOException {
		if (fileType == FileType.BGZ) {
			this.bgzStream = VannoUtils.makeBGZ(path, useJDK);
			this.gzReader = null;
			this.reader = null;
